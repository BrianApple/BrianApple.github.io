...
本文基于 `docs/protocol.md`（**唯一事实来源**）整理，是所有 SDK（Java/Python/Go/C）与 licen-server 的通信契约。服务端字段只增不改、向后兼容；SDK 必须忽略未知字段。
...
License 文件为 JSON，字段顺序固定（与签名 JSON 一致，签名 = 去掉 `sign` 字段后的规范 JSON）：
...
| 字段 | 说明 |
|---|---|
| `licenseId` | 授权编号（如 `LIC-XXXXXXXX-<毫秒时间戳>`，CLI 签发格式） |
| `product` | 产品标识（如 `hxapigate` / `iotgate`） |
| `edition` | 版本/套餐（默认 `enterprise`） |
| `machineCode` | 绑定的机器码（SHA-256 hex） |
| `maxNodes` | 最大并发节点数 |
| `features` | 功能点列表 |
| `issuedAt` / `expiresAt` | 签发/到期时间（ISO-8601） |
| `customer` | 客户名称（可省略） |
| `sign` | RSA 签名（base64） |
...
```
sign = base64( RSA-Sign-PKCS1v15( SHA-256( 去掉 sign 字段后的规范 JSON ) ) )
```

- 使用 RSA-2048 私钥签名（`licen-tool gen-license` / licen-issuer 签发时调用）
- 服务端用内置公钥验签，公钥无法伪造签名
- 改一个字段即验签失败 → `INVALID_SIGNATURE`
...
|---|---|
| `VALID` | 校验通过 |
| `EXPIRED` | 已过期（超过到期时间 + 5 分钟时钟容忍） |
| `NOT_YET_VALID` | 尚未生效（早于签发时间 5 分钟以上） |
| `MACHINE_MISMATCH` | 机器码不匹配（License 绑定的机器码 ≠ 本机） |
| `INVALID_SIGNATURE` | 签名无效（被篡改/伪造） |
| `CLOCK_REVERSED` | 时钟回拨 |
| `MISSING` | License 缺失 |
...
```json
{ "appKey": "hxapigate", "appSecret": "xxx", "nodeId": "uuid-optional（客户端生成，可空则由服务端分配）", "nodeName": "hxapigate-1（可选）", "ip": "10.0.0.5（可选）", "version": "1.0.0（可选）" }
```

**响应 200**：

```json
{ "success": true, "message": "registered", "nodeId": "uuid", "licenseId": "LIC-XXXX", "expiresAt": "2027-08-11T15:32:19+08:00", "onlineNodes": 1, "maxNodes": 10 }
```
...
```json
{ "appKey": "hxapigate", "nodeId": "uuid", "timestamp": "1786433000000", "sign": "hex-hmac" }
```

**响应**：同注册（success=true 表示心跳接受，节点保活）。
...
```json
{ "valid": true, "result": "VALID", "machineCode": "sha256-hex", "licenseId": "LIC-XXXX", "product": "hxapigate", "edition": "enterprise", "customer": "公司一", "maxNodes": 10, "features": ["ai-inference", "nlp"], "issuedAt": "2026-08-11T00:00:00+08:00", "expiresAt": "2027-08-11T00:00:00+08:00" }
```
...
activate 接口不需要管理 Token，但只有**厂商私钥签名 + 绑定本机机器码**的 License 才能激活成功；激活失败不影响当前已激活状态。
